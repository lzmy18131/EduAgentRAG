...
面向 IT 培训技术答疑与求职辅导的多模态 Adaptive RAG 问答系统:技术问题走
「BGE-M3 稠密+稀疏 → RRF → bge-reranker-large → MMR」混合检索,求职问题走
JD 结构化条件检索(意图路由定向触发),报错截图走 Qwen-VL 视觉理解,由 LangGraph
循环做「检索→自省→改写→生成」。

> **核心指标**(453 条合成回归集实测):**HitRate@5 = 0.678**(95%CI 0.6333–0.7191)、
> **MRR = 0.555**;另有 56 条人工编写口语化子集独立报告(分布外验证)。
> 数据规模:**358,221 行向量**(5 语言)+ **46,353 条公开招聘 JD**。
> 完整语料因体积/许可未随仓库分发;评测产物保留在 `2Milvus_RAG_Qa/RAG评测/`,
> 冒烟样例见 `sample_data/`。

**入口**:`run.py` 启动 Web 服务;`MAIN.py` 命令行问答与知识库重建;`app.py` 为 FastAPI
应用(/chat、/chat/stream、/feedback);快速开始见下文。
